...
import { ADD_POINT, ADD_LAYER, GET_LAYERS, GET_POINTS } from '../../queries'
    ...
    const [addLayer] = useMutation<
        ...
        ADD_LAYER,
        {
            update(cache, { data }) {
                ...
                console.log('', data)
                // const newLayer = data?.addTodo.todo
                const existingLayers = cache.readQuery<any>({
                  query: GET_LAYERS,
                })
                console.log('', existingLayers)
        
                /*
                if (existingLayers && newLayer) {
                  cache.writeQuery({
                    query: GET_LAYERS,
                    data: {
                      layers: [
    ...
    const [addPoint] = useMutation<any, any>(
        ADD_POINT,
        {
            update(cache, { data }) {
                ...
                // new value of the GET_ALL_TODOS query.
                console.log('', data)
                //const newPoint = data?.addTodo.todo
                const existingPoints = cache.readQuery<any>({
                  query: GET_POINTS,
                }) 
                console.log('', existingPoints)
        
                /*
                if (existingPoints && newPoint) {
                  cache.writeQuery({
                    query: GET_POINTS,
                    data: {
                      layers: [
